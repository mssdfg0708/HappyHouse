...
export default {
  ...
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      script.src =
        "//dapi.kakao.com/v2/maps/sdk.js?appkey=ea6ad245350957ef798786e3591ab157&libraries=services";
  ...
  methods: {
    initMap() {
      ...
      var marker = new kakao.maps.Marker({
        ...
        title: this.house.title, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
    ...
    displayPlaces(places) {
      ...
      var order = document.getElementById(this.id).getAttribute("data-order");

      for (var i = 0; i < places.length; i++) {
        // 마커를 생성하고 지도에 표시합니다
        var marker = this.addMarker(new kakao.maps.LatLng(places[i].y, places[i].x), order);
        let funcPlaceInfo = this;
        // 마커와 검색결과 항목을 클릭 했을 때
        // 장소정보를 표출하도록 클릭 이벤트를 등록합니다
        (function (marker, place) {
          kakao.maps.event.addListener(marker, "click", function () {
            funcPlaceInfo.displayPlaceInfo(place);
          });
        })(marker, places[i]);
      }
    },
    ...
    onClickCategory(e) {
      if (e.path[0].id == "") {
        var parrent = e.path[1];
      } else {
        var parrent = e.path[0];
      }
      this.id = parrent.id;
      this.className = parrent.children[0].className;
      ...
      if (this.className === "on") {
        ...
        this.changeCategoryClass(parrent);
        this.searchPlaces();
      }
    },
...
#category li.on {
  background: rgba(213, 238, 123, 0.658);
}
#category li:hover {
  background: #ffe6e6;
